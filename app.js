...
(() => {
  ...
  async function apiFetch(url, options = {}) {
    ...
    if (res.status === 401) {
      ...
      // Attempt to silently refresh immediately instead of kicking them out abruptly
      if (tokenClient && localStorage.getItem('yt_explorer_is_logged_in') === 'true') {
         tokenClient.requestAccessToken({ prompt: 'none' });
         showToast("Session refreshing. Please try again in a moment.", "warning");
      } else {
         show(dom.loginOverlay);
         hide(dom.app);
      }
  ...
  function init() {
    ...
    if (isLoggedInUser && restored !== true) {
        // They were logged in, but token is expired or missing.
    ...
    const checkGIS = setInterval(() => {
      if (typeof google !== 'undefined' && google.accounts?.oauth2) {
        ...
        if (isLoggedInUser && restored !== true) {
           ...
           // This will call handleAuthResponse which hides the overlay on success
           tokenClient.requestAccessToken({ prompt: 'none' });
           ...
           setTimeout(() => {
             if (!state.accessToken) {
               console.log("Silent refresh timeout or failed. User needs to login manually.");
